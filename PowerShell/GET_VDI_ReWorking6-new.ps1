<#	
	...
	===========================================================================
	 ...
	 Last Updated:  12/11/2019  17:13 PM
	 Created by:   	RGaines
	 Organization: 	Cloudwave, Inc.
	 Filename:     	GET_VDI-Reworking6-new.ps1
	===========================================================================
	.DESCRIPTION
...
$Pools = $Selector.FindAll() | where {$_.Properties.objectcategory -match "CN=pae-ServerPool" -and $_.Properties.$Pool_ID -and $_.Properties.$object -eq "4"} 
...
Foreach($Pool in $Pools)
   {
    ...
    # Define what value we are looking for
     ...
     #Get Pool Configs
     $maxCount = get-pool -pool_id $Pool |select -ExpandProperty maximumCount
     $minCount = get-pool -pool_id $Pool |select -ExpandProperty minimumCount
     $headroom = get-pool -pool_id $Pool |select -ExpandProperty headroomCount
     ...
     $connected = @(Get-RemoteSession -pool_id $Pool | where {$_.state -eq "Connected"} -ErrorAction SilentlyContinue).count
     $disconnected = @(Get-RemoteSession -pool_id $Pool | where {$_.state -eq "Disconnected"} -ErrorAction SilentlyContinue).count
...
#Checks All VMs 
    ...
    clear-variable dirty, notdirty, ready, clone, maint, provisioned, cust, maintdays, probvms, prepvms, IPMismatch, VcenterIP,FwdNSL,DnsIP,result,PrepParms, PrepvCenterNoIP -ErrorAction SilentlyContinue
    ...
    $Preparing = $cust + $clone + $maint + $PreparingProbs
    ...
    #Perform Utilization Calculations
    $PercentUtilized = [decimal]::Round(($RemoteSessCount/$maxCount) * 100)
    $PercentAvailable = [decimal]::Round(($available/$maxCount) *100)
    $PercentInError = [decimal]::Round(($ProbDesktops/$maxCount) * 100)
...
#CB Session Metrics  
$cbcluster = get-monitor -monitor CBMonitor | where { $_.id -like "*-con0*" } | select -ExpandProperty clusterid
$cbTotalses = get-monitor -monitor CBMonitor | where { $_.id -like "*-con0*" } | select -ExpandProperty totalsessions
$cbTotalHigh = get-monitor -monitor CBMonitor | where { $_.id -like "*-con0*" } | select -ExpandProperty totalsessionshigh
